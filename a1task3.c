...
int hash3(char* s,int i){
    int hash;
    hash = (hash2(s) + i*rehash(s)) % ARRAY_SIZE;
    return hash;
}; //rehash function
...
void insert_entry(struct word_freq table[], char key[], int *cols){
        ...
        int iterations=0;
        int index=hash3(key,iterations);
        //printf("%d %s\n", index, key);
...
int main(void){
    ...
    while(strcmp(input,"quit")!=0){
        ...
        init_input_val = hash3(input,0); //saves initial hash value, will be needed if hash table becomes full
